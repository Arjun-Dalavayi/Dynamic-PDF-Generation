...
public class RequestService {
    ...
    public String checkDb(Request request) throws Exception {

        Query query = new Query();
        query.addCriteria(Criteria.where("seller").is(request.getSeller()));
        query.addCriteria(Criteria.where("sellerGstin").is(request.getSellerGstin()));
        query.addCriteria(Criteria.where("sellerAddress").is(request.getSellerAddress()));
        query.addCriteria(Criteria.where("buyer").is(request.getBuyer()));
        query.addCriteria(Criteria.where("buyerGstin").is(request.getBuyerGstin()));
        query.addCriteria(Criteria.where("buyerAddress").is(request.getBuyerAddress()));
        query.addCriteria(Criteria.where("items").is(request.getItems()));

        List<Request> requests = mongoTemplate.find(query, Request.class);
        if (requests.isEmpty()) {
            return null;
        }
        List<FileName> fileName = fileNameRepository.findByRequestId(requests.get(0));
        File file = new File(fileName.get(0).getFileName());


        if (file.exists()) {
            //It return file path if the file exist in the system
            return file.toString();
        }


        //Download the file again from database if it delete from the local system
        //But this time it don't create another document in database it will remain same
        generatePdf(request, file + "");
        return fileName.get(0).getFileName() + "------its from DB--------";

    }

    private void generatePdf(Request request, String pdfFileName) throws FileNotFoundException, DocumentException {

        System.out.println(pdfFileName + "other generate file");
